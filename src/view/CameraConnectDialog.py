...
class CameraConnectDialog(QDialog, Ui_CameraConnectDialog):
    def __init__(self, parent=None, isStreamSyncEnabled=False):
        ...
        self.resWEdit.setValidator(QRegExpValidator(
            QRegExp("^[0-9]{1,4}$")))  # Integers 0 to 9999
        # resHEdit (resolution: height) input validation
        self.resHEdit.setValidator(QRegExpValidator(
            QRegExp("^[0-9]{1,4}$")))  # Integers 0 to 9999
        ...
        # Set Url Mode
        self.deviceUrlRadioButton.clicked.connect(
    ...
    def openFile(self):
        filename = QFileDialog.getOpenFileName(
            self.parent(), 'open file', '.', 'Excel files(*.mp4 , *.avi)')[0]
