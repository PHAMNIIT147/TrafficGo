...
class CameraView(QWidget, Ui_CameraView):
    ...
    def connectToCamera(self, dropFrameIfBufferFull, apiPreference, capThreadPrio,
        ...
        if self.captureThread.connectToCamera():
            ...
            self.processingThread = ProcessingThread(
                self.sharedImageBuffer, self.deviceUrl, self.cameraId)
            ...
            self.imageBufferBar.setMaximum(
                self.sharedImageBuffer.getByDeviceUrl(self.deviceUrl).maxSize())
    ...
    def updateCaptureThreadStats(self, statData):
        ...
        self.captureRateLabel.setText(
            "{:>6,.2f} fps".format(statData.averageFPS))
    ...
    def newMouseData(self, mouseData):
        ...
        if mouseData.leftButtonRelease and self.frameLabel.pixmap():
            ...
            selectionBox.setWidth(
                wScalingFactor * mouseData.selectionBox.width())
            selectionBox.setHeight(
                hScalingFactor * mouseData.selectionBox.height())
